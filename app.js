...
function normalizeTask(t){return{...t,status:canonicalStatus(t),category:t.category||"其他",updatedAt:t.updatedAt||t.createdAt||Date.now(),id:t.id||crypto.randomUUID()}}
...
$("addBtn").onclick=()=>{const title=$("title").value.trim(),due=$("due").value;if(!title||!due){alert("請輸入事項與期限");return}const d=new Date(due).getTime(),pre=Number($("preReminder").value),now=Date.now();tasks.push({id:crypto.randomUUID(),title,due:d,priority:$("priority").value,category:$("category").value,preReminder:pre,note:$("note").value.trim(),status:"active",remindAt:d-pre*60000,lastNotified:null,createdAt:now,updatedAt:now});save();render();$("title").value="";$("note").value="";$("due").value=localDateInput();scheduleCheck()};
$("list").onclick=e=>{const b=e.target.closest("button");if(!b)return;const el=e.target.closest(".task"),t=tasks.find(x=>x.id===el.dataset.id);if(!t)return;const a=b.dataset.action,now=Date.now();if(a==="done"){t.status="done";t.completedAt=now;t.updatedAt=now}if(a==="nextmonth"){const d=new Date(t.due);d.setMonth(d.getMonth()+1);t.due=d.getTime();t.remindAt=d.getTime()-Number(t.preReminder||0)*60000;t.status="active";t.lastNotified=null;t.updatedAt=now}if(a==="snooze10"){t.remindAt=now+600000;t.lastNotified=null;t.updatedAt=now}if(a==="snooze240"){t.remindAt=now+14400000;t.lastNotified=null;t.updatedAt=now}if(a==="snooze480"){t.remindAt=now+28800000;t.lastNotified=null;t.updatedAt=now}if(a==="custom"){const v=prompt("請輸入下次提醒時間（例如 2026/09/03 18:30）");if(v){const d=new Date(v.replaceAll("/","-"));if(!isNaN(d)){t.remindAt=d.getTime();t.lastNotified=null;t.updatedAt=now}}}if(a==="delete"&&confirm("確定刪除？紀錄會保留在歷史查詢中。")){t.status="deleted";t.deletedAt=now;t.updatedAt=now;t.lastNotified=null}if(a==="restore"){t.status="active";t.deletedAt=null;t.completedAt=null;t.updatedAt=now;if(!t.remindAt||t.remindAt<now)t.remindAt=now+600000;t.lastNotified=null}save();render();scheduleCheck()};
$("historyBtn").onclick=()=>{$("filters").classList.toggle("hidden");if(!$('filters').classList.contains('hidden')){resetFilters();render();$('search').focus()}else render()};
["search","fromDate","toDate","statusFilter","categoryFilter"].forEach(id=>$(id).addEventListener("input",render));["statusFilter","categoryFilter"].forEach(id=>$(id).addEventListener("change",render));$("clearFilter").onclick=()=>{resetFilters();render()};
