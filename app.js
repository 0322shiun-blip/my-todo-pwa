...
tasks=tasks.map(t=>({...t,status:canonicalStatus(t),category:t.category||"其他",updatedAt:t.updatedAt||t.createdAt||Date.now()}));
...
function cls(t){const status=canonicalStatus(t);if(status==="done")return"done";if(status==="deleted")return"deleted";const d=new Date(t.due),n=new Date();if(d<n)return"overdue";if(d.toDateString()===n.toDateString())return"today";return""}
...
function matches(t){
 ...
 const s=$("statusFilter")?.value||"all";if(s!=="all"&&canonicalStatus(t)!==s)return false;
 const c=$("categoryFilter")?.value||"all";if(c!=="all"&&(t.category||"其他")!==c)return false;
 const from=$("fromDate")?.value||"",to=$("toDate")?.value||"";
 const due=new Date(t.due),validDue=!isNaN(due.getTime());
 if(from&&validDue){const d=new Date(from+"T00:00:00");if(due<d)return false}
 if(to&&validDue){const d=new Date(to+"T23:59:59.999");if(due>d)return false}
 return true;
}
...
function render(){
 ...
 const active=tasks.filter(t=>canonicalStatus(t)==="active").sort((a,b)=>new Date(a.due)-new Date(b.due));
